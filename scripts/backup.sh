#!/bin/bash
# Backups of the Lago database, its storage (PDFs, uploads) and the RSA key
# (keys: it signs webhooks and tokens).
#
#   backup.sh            # loop: back up now, then every BACKUP_INTERVAL_HOURS
#   backup.sh now        # one backup
#   backup.sh list       # list backups
#   backup.sh restore <timestamp>   # restore database, storage and keys
#   backup.sh health     # healthcheck: last backup is recent enough
#
# Files: /backups/<timestamp>-db.dump (pg_dump custom format) and
# /backups/<timestamp>-files.tar.gz (storage/ and keys/), deleted after
...
BACKUP_DIR=/backups
DATA_DIR=/data
# Owner of the files: the user Lago runs as.
APP_UID="${LAGO_UID}" APP_GID="${LAGO_UID}"
...
backup() {
    ...
    tar -C "${DATA_DIR}" -czf "${tmp}-files.tar.gz" storage keys
...
restore() {
    ...
    # closes Lago's open connections.
    ...
    echo "==> Restoring storage and keys from ${files}"
    find "${DATA_DIR}/storage" "${DATA_DIR}/keys" -mindepth 1 -maxdepth 1 -exec rm -rf {} +
    tar -C "${DATA_DIR}" -xzpf "${files}"
    chown -R "${APP_UID}:${APP_GID}" "${DATA_DIR}/storage" "${DATA_DIR}/keys"
    echo "==> Restored ${ts}"
}
